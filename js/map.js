...
window.GameMap = (function () {
  ...
  function init(scene) {
    ...
    // South stub split: opening at Z=62→66 for East Discovery Wing corridor
    wall(scene, 25, 58.5,  WALL_T,  7.0, M.galleryWall);  // Z 55→62 (south of discovery wing opening)
    // Gap Z=62→66: East Discovery Wing corridor entrance
    wall(scene, 25, 70.75, WALL_T,  9.5, M.galleryWall);  // Z 66→75.5 (north of discovery wing opening, south of Salon)
    ...
    // Glowing archway at East Discovery Wing entrance (Z=64, east wall of gallery)
    const discoverGlowMat = new THREE.MeshStandardMaterial({
      color: 0x40e8c0, emissive: 0x40e8c0, emissiveIntensity: 1.2,
      roughness: 0.2, transparent: true, opacity: 0.85,
    });
    box(scene, 0.14, 0.14, 4.2, 25, WALL_H + 0.16, 64, discoverGlowMat);          // top bar
    box(scene, 0.14, WALL_H + 0.32, 0.14, 25, WALL_H / 2, 61.9, discoverGlowMat); // south post
    box(scene, 0.14, WALL_H + 0.32, 0.14, 25, WALL_H / 2, 66.1, discoverGlowMat); // north post
    roomLabel(scene, 34, 64, 'Discovery Wing', Math.PI / 2);
